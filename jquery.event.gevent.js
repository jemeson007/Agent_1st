...
(function ( $ ) {
  ...
  $.gevent = (function () {
    ...
    subscribeEvent = (function ( $collection, event_name, fn){
                           $collection.subscribe(event_name, function(request, response){
                        //   let
                        //   $omega={response}, $resource = {}, $projection={}, $ocean= {request}, $m_m={event_name}, $place={place};
                        //   $ocean[request].add($m_m).each(person, function(){
                        //   $resource[$m_m].add($ocean);
                        //   $projection[$omega] = $projection[$resource].add($place);
                        //   response.send($projection[$omega].add($collection));
                        // });   
                        response.contentType('json');
                        response.send({name: 'user', title : 'Event', message: 'Timely'});                            
                      });
