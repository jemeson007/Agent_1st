...
rostrum.shell = (function(){
	...
	var
	  configMap = {
	  	...
	  	+ '<div class="rostrum-shell-head-logo">'
	  	  +'<h1>ROSTRUM</h1>'
	  	  +'<p>Accommodation Solutions</p>'
	  	+'</div>'
	  	+ '<div class="rostrum-shell-head-acct">'
	  	  +'<h2> Sign In</h2>'
	  	  +'</div>'
	  ...
	  setJqueryMap = function(){
	  	...
	  	jqueryMap = {
	  		$container : $container,
	  		$acct : $container.find('.rostrum-shell-head-acct')
	  ...
	  onTapAcct = function(event){
	  	var acct_text, user_name, user = rostrum.model.people.get_user();
	  	if(user.get_is_anon()){
	  		user_name = prompt('Please Sign-in');
	  		rostrum.model.people.login(user_name);
	  		jqueryMap.$acct.text('...processing...');
	  	}
	  	else{
	  		rostrum.model.people.logout();
	  	}
	  	return false;
	  };
...
*/
	  	...
	  	rostrum.avtr.configModule({
	  		chat_model : rostrum.model.chat,
	  		people_model : rostrum.model.people
	  	});
	  	rostrum.avtr.initModule(jqueryMap.$nav);
	  	...
	  	$.gevent.subscribe($container, 'rostrum-login', onLogin);
