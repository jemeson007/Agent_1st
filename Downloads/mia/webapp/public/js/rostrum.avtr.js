 ...
 rostrum.avtr = (function(){
  ...
  var 
    ...
    onTapNav = function(event){
      var css_map,
    	$target = $(event.elem_target).closest('.rostrum-avtr-box');
    ...
    onSetchatee = function(event, arg_map){
    	var
    	  ...
    	  if(old_chatee){
    	  	$nav
    	  	  .find('.rostrum-avtr-box[data-id = ' + old_chatee.cid + ']')
    	  	  .removeClass('rostrum-x-is-chatee');
    	  }
    	  ...
    	  if(new_chatee){
    	  	$nav
    	  	  .find('rostrum-avtr-box[data-id = ' + new_chatee.cid + ']')
    	  	  .addClass('rostrum-x-is-chatee');
    	  }
    };
    	...
    	var
    	  ...
    	  people_db.each(function(person, idx){
    	  	...
    	  	if(person.get_is_anon()){return true;}
    	  		class_list = ['rostrum-avtr-box'];

    	  	if(person.id === chatee.id){
    	  		class_list.push('rostrum-x-is-chatee');
    	  	}
    	  	if(person.get_is_user()){
    	  		class_list.push('rostrum-x-is-user');
    	  	}
    	  	...
    	  	$box = $('<div/>')
    	  	  ...
    	  	  .prop('title', rostrum.util_b.encodeHtml(person.name))
    	  	  .text(person.name)
    	  	  .appendTo($nav);
    	  });
    };
    ...
    // Example : rostrum.avtr.configModule({...});
    ...
    // Example : rostrum.avtr.initModule($container);
    ...
    initModule = function($container){
    	...
    	$.gevent.subscribe($container, 'rostrum-setchatee', onSetchatee);
    	$.gevent.subscribe($container, 'rostrum-listchange', onListchange);
    	$.gevent.subscribe($container, 'rostrum-logout', onLogout);
