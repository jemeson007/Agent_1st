...
roston.shell = (function(){
	...
	var
	  configMap = {
	  	...
	  	resize_interval : 200,	  	
	  	main_html : String()
	  	+ '<div class="roston-shell-head">'
	  	+ '<div class="roston-shell-head-logo">'
	  	  +'<h1>ROSTON</h1>'
	  	  +'<p>Accommodation Solutions</p>'
	  	+'</div>'
	  	+ '<div class="roston-shell-head-acct">'
	  	  +'<h2> Sign In</h2>'
	  	  +'</div>'
	  	+ '<div class="roston-shell-head-search"></div>'
	  	+ '</div>'
	  	+ '<div class="roston-shell-main">'
	  	+ '<div class="roston-shell-main-content"></div>'
	  	+ '</div>'
	  	+ '<div class="roston-shell-foot"></div>'
	  	/*+ '<div class="roston-shell-chat"></div>'*/
	  	+ '<div class="roston-shell-modal"></div>'
	  	+ "/01_01/Dibya/index_p-main"	  
	  },
	  stateMap = {
	  	$container: null,
	  	anchor_map :{},
	  ...
	  onTapAcct, onLogin, onLogout,
	   onResize, onClickChat, initModule;
	  	...
	  	for (key_name in arg_map) {
	  	  if(arg_map.hasOwnProperty(key_name)){
	  	   ...
	  	   try{
	  	   	...
	  	   	$.uriAnchor.setAnchor(stateMap.anchor_map, true, true);
	  	   	bool_return = false;
	  	   }
	  ...
	  setJqueryMap = function(){
	  	...
	  	jqueryMap = {
	  		$container : $container,
	  		//$nav : $nav,
	  		$acct : $container.find('.roston-shell-head-acct')
	  		/*$chat : $container.find('.roston-shell-chat')*/
	  	};
	  };
	  ...
	  onHashChange = function(event){
	  	...
	  	try{anchor_map_proposed = $.uriAnchor.makeAnchorMap();}
	  		...
	  		$.uriAnchor.setAnchor(anchor_map_previous, true, true);
	  		return false;
	  	}
	  	...
	  	if(!anchor_map_previous || _s_chat_previous !== _s_chat_proposed){
	  		...
	  		switch(s_chat_proposed){
	  			case 'opened':
	  			/*toggleChat(true);*/
	  			is_ok = roston.chat.setSliderPosition('opened');
	  			break;
	  			case 'closed':
	  			/*toggleChat(false);*/
	  			is_ok = roston.chat.setSliderPosition('closed');
	  			break;
	  			default :
	  			  // toggleChat(false);
	  			  delete anchor_map_proposed.chat;
	  			  $.uriAnchor.setAnchor(anchor_map_proposed, true, true);
	  		}
	  	}
	  ...
	  // Begin Event handler / Sign in

	  onTapAcct = function(event){
	  	var acct_text, user_name, user = roston.model.people.get_user();
	  	if(user.get_is_anon()){
	  		user_name = prompt('Please Sign-in');
	  		roston.model.people.login(user_name);
	  		jqueryMap.$acct.text('...processing...');
	  	}
	  	else{
	  		roston.model.people.logout();
	  	}
	  	return false;
	  };
	  ...
	  //  Example : roston.shell.InitModule($('#app_div_id'));
	  //  Purpose :
...
*/
	  	...
	  	roston.chat.configModule( {
	  		set_chat_anchor : setChatAnchor,
	  		chat_model : roston.model.chat,
	  		people_model : roston.model.people
	  	} );
	  	roston.chat.initModule( jqueryMap.$container );
	    ...
	    avtr.initModule({
	    	setJqueryMap($stateMap.$container) : $container,
	    	setJqueryMap($configModule(config_map.main_html)) : $container,
	    	setJqueryMap($nav) : $(this)
	    });
	    ...
	    roston.data.initModule();
	  	...
	  	//jquery collection subscribe the onLogin and onLogout event
	  	$.gevent.subscribe($container, 'roston-login', onLogin);
	  	$.gevent.subscribe($container, 'roston-logout', onLogout);

	  	// initialize the user area text
